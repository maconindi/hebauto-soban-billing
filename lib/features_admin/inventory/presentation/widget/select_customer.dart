...
class SearchCustomerDropDown extends StatefulWidget {
  const SearchCustomerDropDown({super.key, required this.customers , required this.customerCallback, this.item = ''});
...
class _SearchCustomerDropDownState extends State<SearchCustomerDropDown> {
  ...
  Widget build(BuildContext context) {
    return DropdownSearch<CustomerEntity>(
      ...
      key: UniqueKey(),
      itemAsString: (CustomerEntity u) => u.email.isEmpty? widget.item : u.email.toString(),
      ...
      onChanged: (data) {
        widget.customerCallback(data  ?? const CustomerEntity());
