...
@injectable
class CustomerTrackingBloc extends Bloc<TrackEventX, CustomerTrackingState> {
  final TrackInventoryUsecase usecase;

  CustomerTrackingBloc(this.usecase) : super(CustomerTrackingState()) {
    on<TrackingCarEvent>(_trackCarEvent);
  }

  Future<void> _trackCarEvent(
      TrackingCarEvent event, Emitter<CustomerTrackingState> emit) async {
    ...
    try {
      ...
      response.fold((error) {
        ...
        if (res.inventId.isNotEmpty) {
          final String userEmail =
              (MainBoxMixin.mainBox?.get(MainBoxKeys.email.name) ?? '');

          if (userEmail == res.cusEmail) {
            emit(state.copyWith(
                success: true, status: TrackingStatus.success, data: res));
          } else {
            emit(state.copyWith(
                success: false,
      ...
      emit(state.copyWith(
          status: TrackingStatus.error, message: exception.toString()));
    }
  }
}
